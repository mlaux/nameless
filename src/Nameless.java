...
public class Nameless extends GridPanel implements KeyListener {	
	...
	private static final int posGuyX = 400;
	private static final int posGuyY = 300;
	...
	public Nameless() {
		...
		bkg = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
	...
	public void paintComponent(Graphics appletGraphics) {
		...
		for(int k = 0; k < numObjects; k++) {
			...
			if(obj[TYPE] == TYPE_EXITPOINT) {
				double dx = obj[X1] - posGuyX / SCALE;
				double dy = obj[Y1] - posGuyY / SCALE;
			...
			double percAcross = 1 - (posGuyX / SCALE - x1) / ((double) width);
			double inside = (minY - (percAcross * height)) - posGuyY / SCALE;
			...
			if (posGuyY/SCALE <= minY + (obj[TYPE]==TYPE_TRIANGLE ? 2000 : 14) && inside < 0) {
			...
			if (collide) {
				...
				if (posGuyX/2 > Math.abs(x2-x1)/2 + Math.min(x1, x2)) {
				...
				if (obj[MOVEX]!=0) {
					if (Math.signum(obj[MOVEX]) == 1 && posGuyX > middle  ||  Math.signum(obj[MOVEX]) == -1 && posGuyX < middle) {
						moveX = obj[MOVEX];
					}
				}
				moveObj(moveX, 0, 0);
	...
	public void drawGuy(Graphics _g, int action, double param, int time, boolean flip) {
		...
		//if (action == WALK || action == STAND) {
			...
			//Bounce effect
			g.rotate(Math.toRadians(-calc),(int)(posGuyX - 4 + Math.sin(Math.toRadians(-calc))*-16), (int)(posGuyY - 35 + Math.cos(Math.toRadians(-calc))*16));
			g.rotate(Math.toRadians(calc),(int)(posGuyX - 4 + Math.sin(Math.toRadians(calc))*-16), (int)(posGuyY - 35 + Math.cos(Math.toRadians(calc))*16));
			
			//back arm
			//if (action==WALK) {
				g.rotate(Math.toRadians(armAngle*1.3), (int)(posGuyX - Math.cos(Math.toRadians(/*bodyAngle+*/90))*35), (int)(posGuyY - 35 - Math.sin(Math.toRadians(/*bodyAngle+*/90))*32));
				g.fillRoundRect((int)(posGuyX - Math.cos(Math.toRadians(/*bodyAngle+*/90))*35) - 3, (int)(posGuyY - 35 - Math.sin(Math.toRadians(/*bodyAngle+*/90))*32), 6, 30, 6, 10);
				g.rotate(Math.toRadians(-armAngle*1.3), (int)(posGuyX - Math.cos(Math.toRadians(/*bodyAngle+*/90))*35), (int)(posGuyY - 35 - Math.sin(Math.toRadians(/*bodyAngle+*/90))*32));
			//} else {
			...
			//back leg1
			g.rotate(Math.toRadians(-calc),(int)posGuyX, (int)posGuyY - 35);
			g.fillRoundRect((int)posGuyX - 4, (int)posGuyY - 35, 7, 18, 5, 10);
			g.rotate(Math.toRadians(calc),(int)posGuyX, (int)posGuyY - 35);
			
			//back leg2
			g.rotate(Math.toRadians(-calc+15),(int)(posGuyX - 4 + Math.sin(Math.toRadians(-calc+15))*16 + 2), (int)(posGuyY - 35 + Math.cos(Math.toRadians(-calc+15))*16) + 2);
			g.fillRoundRect((int)(posGuyX - 4 + Math.sin(Math.toRadians(calc+15))*16) - 2, (int)(posGuyY - 35 + Math.cos(Math.toRadians(calc+15))*16) + 2, 6, 20, 6, 10);
			g.rotate(Math.toRadians(calc-15),(int)(posGuyX - 4 + Math.sin(Math.toRadians(-calc+15))*16 + 2), (int)(posGuyY - 35 + Math.cos(Math.toRadians(-calc+15))*16) + 2);
			
			//body
			//g.rotate(Math.toRadians(bodyAngle), (int)posX, (int)posY - 35);
			g.fillRoundRect((int)posGuyX - 8, (int)(posGuyY - 72), 15, 40, 10, 15);
			
			//head
			g.fillRoundRect((int)posGuyX - 12, (int)(posGuyY - 96), 22, 25, 20, 20);
			//g.rotate(Math.toRadians(-bodyAngle), (int)posX, (int)posY - 35);
